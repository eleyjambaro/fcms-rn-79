...
import StockUsageList from '../components/purchases/StockUsageList';
...
const StockUsage = props => {
  ...
  useEffect(() => {
    if (route.params?.batchStockUsageSuccess) {
      showSuccessDialog();
    }
  }, [route.params?.batchStockUsageSuccess]);
    ...
    <>
      <Portal>
        <Dialog visible={successDialogVisible} onDismiss={hideSuccessDialog}>
          ...
          <Dialog.Content>
            <Paragraph>
              Your stock usages have been successfully logged and made changes
              to your inventory
            </Paragraph>
          </Dialog.Content>
