...
import EndingInventoryReportFileExportForm from '../forms/EndingInventoryReportFileExportForm';
import {
  getRevenueGroups,
...
import {createInventoryOperationsReportWorksheet} from './ReportsFileExportWorksheets';
import {getCompany} from '../../localDbQueries/companies';

const EndingInventoryReportFileExport = props => {
  ...
  const fileName = `Ending Inventory - ${selectedMonthAndYear}`;
  ...
  const exportDataToExcel = async options => {
    ...
    const companyName = company.company_display_name
      ...
      : '';
    const branch = company.branch ? company.branch : '';
    ...
    const itemsSheetTitle = [`${title}`];
    const itemsSheetSubtitle = [`Ending Inventory: ${selectedMonthAndYear}`];
    ...
    revenueGroupsDataToExport.forEach(item => {
      ...
      revenueGroupsTable.push([
        item.name,
        `${currencySymbol} ${commaNumber(totalRevenue.toFixed(2))}`,
    ...
    for (let key in categoriesByRevenueGroups) {
      ...
      categoriesTable.push([
        ...
        `${currencySymbol} ${commaNumber(
          (
            revenueGroupsTotals?.revenueGroups?.[key]
  ...
  const renderExportOptionsModalContent = () => {
    ...
    return (
      <EndingInventoryReportFileExportForm
        editMode={true}
...
export default EndingInventoryReportFileExport;
