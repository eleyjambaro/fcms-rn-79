...
import RecipeReportFileExportForm from '../forms/RecipeReportFileExportForm';
import {
  getRevenueGroups,
...
import {
  getAllRecipesWithAllIngredients,
  getAllRecipesWithAllIngredientsTotal,
...
const RecipeReportFileExport = props => {
  ...
  const {
    data: getAllRecipesWithAllIngredientsData,
    status: getAllRecipesWithAllIngredientsStatus,
    error: getAllRecipesWithAllIngredientsError,
    refetch: refetchAllRecipesWithAllIngredients,
  } = useQuery(
    [
      'allRecipesWithAllIngredients',
      {
        selectedDateFilter,
        selectedMonthYearDateFilter,
        exactDateFilter,
  ...
  const {
    data: getAllRecipesWithAllIngredientsTotalData,
    status: getAllRecipesWithAllIngredientsTotalStatus,
    error: getAllRecipesWithAllIngredientsTotalError,
    refetch: refetchAllRecipesWithAllIngredientsTotal,
  } = useQuery(
    [
      'allRecipesWithAllIngredientsTotal',
      {
        selectedMonthYearDateFilter,
        selectedMonthYearDateFilter,
        exactDateFilter,
  ...
  const fileName = `Recipes and Ingredients - ${fileNameDate}`;
  ...
  const exportDataToExcel = async options => {
    if (getAllRecipesWithAllIngredientsStatus === 'loading') {
      return;
    }

    if (getAllRecipesWithAllIngredientsTotalStatus === 'loading') {
      return;
    }

    if (getAllRecipesWithAllIngredientsStatus === 'error') {
      return;
    }

    if (getAllRecipesWithAllIngredientsTotalStatus === 'error') {
      return;
    }
    ...
    let allRecipesWithAllIngredientsDataToExport =
      getAllRecipesWithAllIngredientsData?.result;
    let allRecipesWithAllIngredientsTotalData =
      getAllRecipesWithAllIngredientsTotalData?.resultMap;
    ...
    const allRecipesWithAllIngredientsSheetTitle = [
      `Recipes and Ingredients: ${fileNameDate}`,
    ];

    let allRecipesWithAllIngredientsSheetTable1 = [
      [`Report Generation Date: ${currentDate}`],
      [''],
    ];

    let allRecipesWithAllIngredientsSheetTable1Totals = [['']];

    // allRecipesWithAllIngredientsSheetTable1Totals.push([
    //   '',
    //   '',
    //   '',
    //   '',
    ...
    let recipeOrItemNameColWidth = 20;
    let recipeCount = 0;
    let currentRecipeId = null;
    let previousRecipeId = null;
    let lastRecipeIndex = allRecipesWithAllIngredientsDataToExport?.length - 1;

    allRecipesWithAllIngredientsDataToExport.forEach((item, index) => {
      // row with custom height
      eachItemRow.push({hpx: 20});

      if (item.recipe_name?.length > recipeOrItemNameColWidth) {
        recipeOrItemNameColWidth = item.recipe_name.length;
      }

      if (item.item_name?.length > recipeOrItemNameColWidth) {
        recipeOrItemNameColWidth = item.item_name.length;
      }

      // increase recipe count
      if (!currentRecipeId || currentRecipeId !== item.recipe_id) {
        recipeCount++;
      }

      if (!currentRecipeId || currentRecipeId !== item.recipe_id) {
        if (currentRecipeId && currentRecipeId !== item.recipe_id) {
          // all ingredients total

          previousRecipeId =
            allRecipesWithAllIngredientsDataToExport[index - 1]?.recipe_id;

          allRecipesWithAllIngredientsSheetTable1.push(['']);
          allRecipesWithAllIngredientsSheetTable1.push([
            '',
            '',
            'Ingredients Total (Net)',
            `${currencySymbol} ${commaNumber(
              (
                allRecipesWithAllIngredientsTotalData[previousRecipeId]
                  ?.recipe_all_ingredients_total_cost_net || 0
              )?.toFixed(2),
            )}`,
          ]);

          // space below each recipe
          allRecipesWithAllIngredientsSheetTable1.push(['']);
          allRecipesWithAllIngredientsSheetTable1.push(['']);
          allRecipesWithAllIngredientsSheetTable1.push(['']);
        }
      ...
      if (currentRecipeId === item.recipe_id) {
        allRecipesWithAllIngredientsSheetTable1.push([
          '',
          item.item_name,
          `${commaNumber(
            (parseFloat(item.in_recipe_qty) || 0).toFixed(2),
          )} ${formatUOMAbbrev(item.in_recipe_uom_abbrev)}`,
          `${currencySymbol} ${commaNumber(
      ...
      // last index
      if (index === lastRecipeIndex) {
        // all ingredients total for the last index

        const recipeId =
          allRecipesWithAllIngredientsDataToExport[index]?.recipe_id;
        ...
        allRecipesWithAllIngredientsSheetTable1.push([
          ...
          `${currencySymbol} ${commaNumber(
            (
              allRecipesWithAllIngredientsTotalData[recipeId]
                ?.recipe_all_ingredients_total_cost_net || 0
            )?.toFixed(2),
          )}`,
        ]);
    ...
    allRecipesWithAllIngredientsDataToExport = [
      allRecipesWithAllIngredientsSheetTitle,
      ...allRecipesWithAllIngredientsSheetTable1,
      ...allRecipesWithAllIngredientsSheetTable1Totals,
    ];

    let allRecipesWithAllIngredientsWorksheet = XLSX.utils.aoa_to_sheet(
      allRecipesWithAllIngredientsDataToExport,
    );

    const allRecipesWithAllIngredientsWsCols = [
      {wch: 10}, // A
      {wch: recipeOrItemNameColWidth}, // B
      {wch: 20}, // C
      {wch: 20}, // D
      {wch: 20}, // E
      {wch: 20}, // F
    ...
    allRecipesWithAllIngredientsWorksheet['!cols'] =
      allRecipesWithAllIngredientsWsCols;

    // const allRecipesWithAllIngredientsWsRows = [
    //   {hpx: 26},
    //   {hpx: 20},
    //   {hpx: 20},
    //   {hpx: 20},
    ...
    //   {hpx: 20},
    // ];
    // allRecipesWithAllIngredientsWorksheet['!rows'] =
    //   allRecipesWithAllIngredientsWsRows;

    // Merge Cells
    // if (!allRecipesWithAllIngredientsWorksheet['!merges']) allRecipesWithAllIngredientsWorksheet['!merges'] = [];
    // allRecipesWithAllIngredientsWorksheet['!merges'].push(XLSX.utils.decode_range('A1:B1'));
    // allRecipesWithAllIngredientsWorksheet['!merges'].push(XLSX.utils.decode_range('A2:B2'));
    // allRecipesWithAllIngredientsWorksheet['!merges'].push(XLSX.utils.decode_range('A4:A5'));
    // allRecipesWithAllIngredientsWorksheet['!merges'].push(XLSX.utils.decode_range('B4:B5'));
    // allRecipesWithAllIngredientsWorksheet['!merges'].push(XLSX.utils.decode_range('C4:F4'));
    // allRecipesWithAllIngredientsWorksheet['!merges'].push(XLSX.utils.decode_range('G4:J4'));
    // allRecipesWithAllIngredientsWorksheet['!merges'].push(XLSX.utils.decode_range('K4:M4'));
    ...
    //   XLSX.utils.book_append_sheet(workbook, allRecipesWithAllIngredientsWorksheet, 'Items');
    // }

    XLSX.utils.book_append_sheet(
      workbook,
      allRecipesWithAllIngredientsWorksheet,
