...
import StockUsageHistoryListItem from './StockUsageHistoryListItem';
...
import {getBatchStockUsageGroups} from '../../localDbQueries/batchStockUsage';

const StockUsageHistoryList = props => {
  const {filter} = props;
  ...
  const {
    ...
    ['batchStockUsageGroups', {filter}],
    getBatchStockUsageGroups,
    {
  ...
  const renderItem = ({item}) => {
    return (
      <StockUsageHistoryListItem
        item={item}
        onPress={() => {
          navigation.navigate('StockUsageHistoryView', {
            stockUsageList: item,
            batch_stock_usage_group_id: item.id,
          });
        }}
      />
    );
...
export default StockUsageHistoryList;
