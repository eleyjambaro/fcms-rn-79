...
import {
  ...
  getCategoriesCustomReport,
  getCategoriesCustomReportTotals,
...
const CategoryCustomReportTabularList = props => {
  const {
    dateFilter,
    highlightedItemId,
    filter = {},
  ...
  const {
    data: categoriesCustomReportTotalsData,
    status: categoriesMonthlyReportTotalsStatus,
  } = useQuery(
    [
      'categoriesCustomReportTotals',
      {
        dateFilter,
        monthYearDateFilter,
        selectedMonthYearDateFilter,
  ...
  const totals = categoriesCustomReportTotalsData?.totals;
  ...
  const renderItem = ({item, index, tableColumnNumber}) => {
    ...
    const dateFilteredGrandTotalCost = item.date_filtered_grand_total_cost || 0;
    const dateFilteredGrandTotalCostNet =
      item.date_filtered_grand_total_cost_net || 0;
    const dateFilteredGrandTotalCostTax =
      item.date_filtered_grand_total_cost_tax || 0;
      ...
      return (
        <DataTable.Row style={[tableRowStyle]}>
          <DataTable.Cell numeric>{`${currencySymbol} ${commaNumber(
            dateFilteredGrandTotalCost.toFixed(2),
          )}`}</DataTable.Cell>
          <DataTable.Cell numeric>{`${currencySymbol} ${commaNumber(
            dateFilteredGrandTotalCostTax.toFixed(2),
          )}`}</DataTable.Cell>
          <DataTable.Cell numeric>{`${currencySymbol} ${commaNumber(
            dateFilteredGrandTotalCostNet.toFixed(2),
          )}`}</DataTable.Cell>
        </DataTable.Row>
      );
    }
          ...
          <DataTable.Cell numeric>
            <Text style={{fontWeight: 'bold'}}>
              {`${currencySymbol} ${commaNumber(
                (totals?.dateFilteredAllCategoriesTotalCost || 0).toFixed(2),
              )}`}
            </Text>
          </DataTable.Cell>
          <DataTable.Cell numeric>
            <Text style={{fontWeight: 'bold'}}>
              {`${currencySymbol} ${commaNumber(
                (totals?.dateFilteredAllCategoriesTotalCostTax || 0).toFixed(2),
              )}`}
            </Text>
          </DataTable.Cell>
          <DataTable.Cell numeric>
            <Text style={{fontWeight: 'bold'}}>
              {`${currencySymbol} ${commaNumber(
                (totals?.dateFilteredAllCategoriesTotalCostNet || 0).toFixed(2),
              )}`}
            </Text>
          </DataTable.Cell>
        </DataTable.Row>
...
export default CategoryCustomReportTabularList;
