...
const SellingMenuItemUnitAndQuantityValidationSchema = Yup.object().shape({
  use_measurement_per_piece: Yup.boolean(),
  in_menu_qty: Yup.string().required('Required'),
  in_menu_uom_abbrev: Yup.string().required('Required'),
});

const SellingMenuItemUnitAndQuantityForm = props => {
  const {
    initialValues = {
  ...
  const [unit, setUnit] = useState(initialValues.in_menu_uom_abbrev);
        ...
        <View style={{marginVertical: 10}}>
          <ConfirmationCheckbox
            ...
            onPress={() => {
              ...
              setFieldTouched('in_menu_uom_abbrev', true);
              setFieldValue('in_menu_uom_abbrev', uomAbbrevValue);
    ...
    <Formik
      initialValues={{
        use_measurement_per_piece:
          ...
          (item?.uom_abbrev === 'ea' &&
          initialValues.in_menu_uom_abbrev !== 'ea'
            ? true
            : false) ||
          false,
        in_menu_qty: initialValues.in_menu_qty?.toString(),
        in_menu_uom_abbrev: initialValues.in_menu_uom_abbrev || 'kg',
      }}
      onSubmit={onSubmit}
      validationSchema={SellingMenuItemUnitAndQuantityValidationSchema}>
      {props => {
        const {
          handleChange,
          handleBlur,
        ...
        return (
          <>
            <View style={{flexDirection: 'row'}}>
              <TextInput
                label="Quantity"
                onChangeText={handleChange('in_menu_qty')}
                onBlur={handleBlur('in_menu_qty')}
                value={values.in_menu_qty}
                style={[styles.textInput, {flex: 1}]}
                autoFocus={true}
                keyboardType="numeric"
                error={errors.in_menu_qty && touched.in_menu_qty ? true : false}
              />
              <QuantityUOMText
                uomAbbrev={unit}
                quantity={values.in_menu_qty}
                operationType="add"
              />
            </View>
            <Dropdown
              ...
              onSelect={value => {
                setUnit(value);
                handleChange('in_menu_uom_abbrev')(value);
              }}
              options={unitOptions}
...
export default SellingMenuItemUnitAndQuantityForm;
