...
const ExpenseGroupListItem = props => {
  const {item, onPress, onPressItemOptions, viewMode = 'list'} = props;
        ...
        {viewMode === 'list' && (
          <View
            ...
            <Text>{`${currencySymbol} ${commaNumber(
              item.total_expense?.toFixed(2) || 0,
            )}`}</Text>
          </View>
        )}
      </View>
...
export default ExpenseGroupListItem;
