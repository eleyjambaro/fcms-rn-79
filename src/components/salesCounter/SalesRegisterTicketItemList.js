...
import SalesRegisterTicketItemListItem from './SalesRegisterTicketItemListItem';
...
const SalesRegisterTicketItemList = props => {
  ...
  const handleSubmit = values => {
    actions?.updateSaleItemFromSalesRegisterTicket(
      values,
      values.sale_qty,
      false,
    );
  ...
  const handleRemoveItemFromSalesRegisterTicket = values => {
    actions?.removeSaleItemFromSalesRegisterTicket(
      values, // item
    );
  ...
  const renderItem = ({item}) => {
    const saleQty = saleItems?.[item.saleItemRefId]?.saleQty;

    return (
      <SalesRegisterTicketItemListItem
        isHighlighted={
          item?.saleItemRefId === focusedItem?.saleItemRefId ? true : false
        }
        item={item}
        saleQty={saleQty}
        ...
        <Button
          ...
          onPress={() => {
            ...
            navigation.navigate(routes.paymentMethod(), {
              ...
              route_to_go_back: routeToGoBack,
            });
          }}>
          Checkout
        </Button>
  ...
  return (
    <>
      ...
      <FlatList
        ...
        data={items}
        keyExtractor={item => item.saleItemRefId}
...
export default SalesRegisterTicketItemList;
